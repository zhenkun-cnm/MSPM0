...
- Added target yaw-rate slew limiting and filtered gyro feedback for smoother straight-line and corner transitions.
- Added TFT `PID -> GrayLine` entries: `RateKp`, `RateKi`, `RateKd`, `RateMax`, `RateTrim`, and `RateSlew`.
...
Next validation: capture 10-20 seconds of JustFloat for straight, continuous curve, and sharp curve tests, then tune `RateKp` and `RateTrim` first.
...
## 2026-07-24 - Logging management completed

- Use `log status` to inspect the active runtime level, module mask, and telemetry source.
- Use `log level debug` for throttled diagnostic records and `log module <name> on|off` to isolate one module.
- Use `log telemetry motion`, `log telemetry grayline`, or `log telemetry off`; telemetry is off after reset.
